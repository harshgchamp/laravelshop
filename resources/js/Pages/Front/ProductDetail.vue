...
import ProductGrid from '@/Pages/Front/Components/Product/ProductGrid.vue'
      ...
      >
        ...
        <div class="mx-auto px-5 lg:px-5">
          ...
          <p class="mt-5 font-bold">
            Availability: <span class="text-green-600">{{ product.in_stock > 0 ? 'In Stock' : 'Out of Stock' }}</span>
          </p>
          ...
          <p class="mt-4 text-4xl font-bold text-violet-900">
            ${{ product.discount_price ? product.discount_price : product.price }} <span v-if="product.discount_price" class="text-xs text-gray-400 line-through">${{ product.price  }}</span>
          </p>

          <p class="pt-5 text-sm leading-5 text-gray-500 text-justify"  v-html="product.description"></p>
   
          <div class="mt-7 flex flex-row items-center gap-6" v-if="product.in_stock > 0">
