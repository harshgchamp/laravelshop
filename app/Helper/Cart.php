...
class Cart
{
    ...
    {
        if ($user = auth()->id()) {
            return CartItem::whereUserId($user)->count();
        } else {
            return array_reduce(
    ...
    {
        return json_decode(request()->cookie('cart_items_hghg', '[]'), true);
    }
    ...
    {
        $userId = auth()->id();
        $userCartItems = CartItem::where(['user_id' => $userId])->get()->keyBy('product_id');
        $savedCartItems = [];
